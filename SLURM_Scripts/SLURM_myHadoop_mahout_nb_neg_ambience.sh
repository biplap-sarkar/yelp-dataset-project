...
#SBATCH --job-name=mahout_nb_neg_ambience
#SBATCH --mail-user=biplapsa@buffalo.edu
#SBATCH --output=nb_neg_ambience.out
#SBATCH --error=nb_neg_ambience.out
...
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR dfs -put ./sentiments-seq /data
sleep 15
...
##### this will generate files in HDFS in the directory /data/sentiments-vectors#######
$MAHOUT_HOME/bin/mahout seq2sparse -i /data/sentiments-seq -o /data/sentiments-vectors
echo "-------list directory sentiment-vectors ---"
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR dfs -ls /data/sentiments-vectors
...
$MAHOUT_HOME/bin/mahout split -i /data/sentiments-vectors/tfidf-vectors --trainingOutput /data/train-vectors --testOutput /data/test-vectors --randomSelectionPct 20 --overwrite --sequenceFiles -xm sequential
...
$MAHOUT_HOME/bin/mahout testnb -i /data/train-vectors -m model -l labelindex -ow -o /data/sentiments-testing -c
echo "------- To test the classifier is working properly on testing set ---"
$MAHOUT_HOME/bin/mahout testnb -i /data/test-vectors -m model -l labelindex -ow -o sentiments-testing -c
